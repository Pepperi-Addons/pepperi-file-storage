...
export class OfflineTests extends ABasePfsTests
{
	title = 'Offline tests';

	protected readonly pfsSchemaName = `PfsOfflineTest`;
	protected readonly testFileName = 'test.png';
	...
	): void 
	{
		describe(this.title, () => 
		{
			...
			{
				await this.ensureSchemaDoesntExist();
			...
			it('Post a file online and sync it', async () => 
			{
				...
				// Make sure it is synced
				const offlineFile = await this.pfsOfflineService.getByKey(this.pfsSchemaName, this.testFileName);
				...
				expect(offlineFile.URL).to.match(/localhost|\.pepperi\.com\/temp/i)
			...
			{
				// Get file online
				const onlineFileBeforeDeletion = await this.pfsOnlineService.getByKey(this.pfsSchemaName, this.testFileName);
				...
				const onlineFileAfterDeletion = await this.pfsOnlineService.getByKey(this.pfsSchemaName, this.testFileName);
	 ...
	 */
	protected async ensureSchemaDoesntExist()
	{
